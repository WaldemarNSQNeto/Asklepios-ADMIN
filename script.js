...
function gerarSintese() {
    ...
    //TAPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
    ...
    //GLICEMIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
    ...
    //UREIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
    ...
    const cloroFormatada = ` Cl ${cloro} //<br>`;
    ...
    const gasometriaCampos = [
        ...
        { id: 'hco3', label: 'HCO3' },
        { id: 'be', label: 'BE (B)' },
        { id: 'sato2', label: 'SatO2' },
        { id: 'lac', label: 'Lac' },
    ];

    let gasometriaFormatada = `<b>GASOMETRIA ARTERIAL:</b> `;
    let algumValorGasometriaEncontrado = false;

    gasometriaCampos.forEach((campo, index) => {
        const elemento = document.getElementById(campo.id);
        const valor = elemento ? elemento.textContent.trim() : 'Não encontrado';
        if (valor !== 'Não encontrado') {
            algumValorGasometriaEncontrado = true;
            gasometriaFormatada += `${campo.label}: ${valor}`;
            if (index < gasometriaCampos.length - 1) {
                gasometriaFormatada += ' / ';
            }
        }
    });

    if (algumValorGasometriaEncontrado) {
        gasometriaFormatada += ' //<br>';
        sinteseHTML += gasometriaFormatada;
    } else {
        sinteseHTML += `<b>GASOMETRIA ARTERIAL:</b> Não encontrado<br>`;
    }

    // Captura e formata os dados da Bioquímicaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa

    // Captura e formata os dados da Gasometria
    const gasometriaElement = document.getElementById('gasometria');
    const gasometria = gasometriaElement ? gasometriaElement.innerHTML : 'Não encontrado';
    const gasometriaVenosaElement = document.getElementById('gasometriaVenosa');
    const gasometriaVenosa = gasometriaVenosaElement ? gasometriaVenosaElement.innerHTML : 'Não encontrado';

    // Formata os dados da Gasometria
    const gasometriaFormatada = `<b>GASOMETRIA ARTERIAL:</b> [${gasometria}]<br>`;
    adicionarExame('gasometria', gasometria, gasometriaFormatada);
    const gasometriaVenosaFormatada = `<b>GASOMETRIA VENOSA:</b> [${gasometriaVenosa}]<br>`;
    ...
    // Captura e formata os dados do CPK
    ...
    const CPKFormatado = `<b>CPK:</b> [${CPK}]<br>`;
    adicionarExame('CPK', CPK, CPKFormatado);

    // Captura e formata os dados do D-Dímero
    ...
    const ddimeroFormatado = `<b>D-DÍMERO:</b> [${ddimero}]<br>`;
    adicionarExame('ddimero', ddimero, ddimeroFormatado);

    // Captura e formata os dados do CK-MB
    ...
    const CKMBFormatado = `<b>CK-MB:</b> [${CKMB}]<br>`;
    adicionarExame('CKMB', CKMB, CKMBFormatado);

    // Captura e formata os dados do NT-proBNP
    ...
    const NTproBNPFormatado = `<b>NT-proBNP:</b> [${NTproBNP}]<br>`;
    adicionarExame('NTproBNP', NTproBNP, NTproBNPFormatado);

    // Captura e formata os dados do LDH
    ...
    const LDHFormatado = `<b>LDH:</b> [${LDH}]<br>`;
    adicionarExame('LDH', LDH, LDHFormatado);

    // Captura e formata os dados da Ferritina
    ...
    const ferritinaFormatada = `<b>FERRITINA:</b> [${ferritina}]<br>`;
    adicionarExame('ferritina', ferritina, ferritinaFormatada);

    // Captura e formata os dados do Ferro Sérico
    ...
    const ferrosericoFormatado = `<b>FERRO SÉRICO:</b> [${ferroserico}]<br>`;
    adicionarExame('ferroserico', ferroserico, ferrosericoFormatado);
    ...
    const proteinatotalfracoes = proteinatotalfracoesElement ? proteinatotalfracoesElement.innerHTML : 'Não encontrado';

    // Formata os dados da Proteína Total e Frações
    const proteinatotalfracoesFormatada = `<b>PROTEÍNA TOTAL E FRAÇÕES:</b> [${proteinatotalfracoes}]<br>`;
    adicionarExame('proteinatotalfracoes', proteinatotalfracoes, proteinatotalfracoesFormatada);

    // Captura e formata os dados da Troponina I
    ...
    const troponinaiFormatada = `<b>TROPONINA I:</b> [${troponinai}]<br>`;
    adicionarExame('troponinai', troponinai, troponinaiFormatada);

    // Captura e formata os dados do UPH
    ...
    const uphFormatado = `<b>UREIA PÓS-HEMODIÁLISE:</b> [${uph}]<br>`;
    adicionarExame('uph', uph, uphFormatado);

    // Captura e formata os dados da FA
    const FAElement = document.getElementById('FA');
    const FA = FAElement ? FAElement.textContent : 'Não encontrado';

    // Formata os dados da FA
    const FAFormatado = `<b>FOSFATASE ALCALINA:</b> [${FA}]<br>`;
    adicionarExame('FA', FA, FAFormatado);

    // Captura e formata os dados do IST
    ...
    const istFormatado = `<b>ÍNDICE SAT. TRANSFERRINA:</b> [${ist}]<br>`;
    adicionarExame('ist', ist, istFormatado);

    // Captura e formata os dados do T4
    const T4Element = document.getElementById('T4');
    const T4 = T4Element ? T4Element.textContent : 'Não encontrado';

    // Formata os dados do T4
    const T4Formatado = `<b>T4 LIVRE:</b> [${T4}]<br>`;
    adicionarExame('T4', T4, T4Formatado);

    // Captura e formata os dados do TSH
    ...
    const TSHFormatado = `<b>TSH:</b> [${TSH}]<br>`;
    adicionarExame('TSH', TSH, TSHFormatado);
    ...
    const vitaminaDFormatada = `<b>VITAMINA D:</b> [${vitaminaD}]<br>`;
    ...
    const vitB12Formatada = `<b>VITAMINA B12:</b> [${vitB12}]<br>`;
    ...
    const PTHintactoFormatado = `<b>PTH INTACTO:</b> [${PTHintacto}]<br>`;
    adicionarExame('PTHintacto', PTHintacto, PTHintactoFormatado);
    ...
    const RCFormatado = `<b>RITMO DE CORTISOL:</b> [${RC}]<br>`;
    adicionarExame('RC', RC, RCFormatado);
    ...
    const gamaGGTFormatado = `<b>GAMA GGT:</b> [${gamaGGT}]<br>`;
    adicionarExame('gamaGGT', gamaGGT, gamaGGTFormatado);
    ...
    const lipaseFormatado = `<b>LIPASE:</b> [${lipase}]<br>`;
    adicionarExame('lipase', lipase, lipaseFormatado);
    ...
    const amilaseFormatado = `<b>AMILASE:</b> [${amilase}]<br>`;
    adicionarExame('amilase', amilase, amilaseFormatado);
    ...
    const fibrinogenioFormatado = `<b>FIBRINOGÊNIO:</b> [${fibrinogenio}]<br>`;
    adicionarExame('fibrinogenio', fibrinogenio, fibrinogenioFormatado);

    // Captura e formata os dados do Resultado de Antígenos do SARS-CoV-2
    ...
    const RASARSCoV2Formatado = `<b>RESULTADO DE ANTÍGENOS DO SARS-CoV-2:</b> [${RASARSCoV2}]<br>`;
    adicionarExame('RASARSCoV2', RASARSCoV2, RASARSCoV2Formatado);

    // Captura e formata os dados do ANTI-HBc IgM (HEPATITE B)
    ...
    const antihbcigmFormatado = `<b>ANTI-HBc IgM (HEPATITE B):</b> [${antihbcigm}]<br>`;
    adicionarExame('antihbcigm', antihbcigm, antihbcigmFormatado);

    // Captura e formata os dados do ANTI-HBc TOTAL (HEPATITE B)
    ...
    const antihbctotalFormatado = `<b>ANTI-HBc TOTAL (HEPATITE B):</b> [${antihbctotal}]<br>`;
    adicionarExame('antihbctotal', antihbctotal, antihbctotalFormatado);

    // Captura e formata os dados do ANTI-HBS (HEPATITE B)
    ...
    const antihbsFormatado = `<b>ANTI-HBS (HEPATITE B):</b> [${antihbs}]<br>`;
    adicionarExame('antihbs', antihbs, antihbsFormatado);

    // Captura e formata os dados da Detecção de Antígeno de Superfície do Vírus da Hepatite B (HBS-Ag)
    ...
    const dasvhbFormatado = `<b>DETECÇÃO DE ANTÍGENO DE SUPERFÍCIE DO VÍRUS DA HEPATITE B (HBS-Ag):</b> [${dasvhb}]<br>`;
    adicionarExame('dasvhb', dasvhb, dasvhbFormatado);

    // Captura e formata os dados do HBsAg (HEPATITE B)
    ...
    const hbsagFormatado = `<b>HBsAg (HEPATITE B):</b> [${hbsag}]<br>`;
    adicionarExame('hbsag', hbsag, hbsagFormatado);

    // Captura e formata os dados da Detecção de Anticorpo contra o vírus da hepatite C (anti-HCV)
    ...
    const dacvhcFormatado = `<b>DETECÇÃO DE ANTICORPO CONTRA O VÍRUS DA HEPATITE C (anti-HCV):</b> [${dacvhc}]<br>`;
    adicionarExame('dacvhc', dacvhc, dacvhcFormatado);

    // Captura e formata os dados do ANTI-HCV (HEPATITE C)
    ...
    const antihcvFormatado = `<b>ANTI-HCV (HEPATITE C):</b> [${antihcv}]<br>`;
    adicionarExame('antihcv', antihcv, antihcvFormatado);

    // Captura e formata os dados do VDRL
    ...
    const VDRLFormatado = `<b>VDRL:</b> [${VDRL}]<br>`;
    adicionarExame('VDRL', VDRL, VDRLFormatado);

    // Captura e formata os dados da Detecção de Anticorpos ANTI-HIV 1 E 2
    ...
    const daANTIHIV1e2Formatado = `<b>DETECÇÃO DE ANTICORPOS ANTI-HIV 1 E 2:</b> [${daANTIHIV1e2}]<br>`;
    adicionarExame('daANTIHIV1e2', daANTIHIV1e2, daANTIHIV1e2Formatado);

    // Captura e formata os dados do HIV 1 e 2, ANTÍGENO/ANTICORPOS
    ...
    const hiv1e2Formatado = `<b>HIV 1 e 2, ANTÍGENO/ANTICORPOS:</b> [${hiv1e2}]<br>`;
    adicionarExame('hiv1e2', hiv1e2, hiv1e2Formatado);

    // Captura e formata os dados do Anticorpo IgG contra Treponema Pallidum
    ...
    const aIgGcTPFormatado = `<b>ANTICORPO IgG CONTRA TREPONEMA PALLIDUM:</b> [${aIgGcTP}]<br>`;
    adicionarExame('aIgGcTP', aIgGcTP, aIgGcTPFormatado);

    // Captura e formata os dados do IGA
    ...
    const IGAFormatado = `<b>IMUNOGLOBULINA (IGA):</b> [${IGA}]<br>`;
    adicionarExame('IGA', IGA, IGAFormatado);

    // Captura e formata os dados do IGM
    ...
    const IGMFormatado = `<b>IMUNOGLOBULINA (IGM):</b> [${IGM}]<br>`;
    adicionarExame('IGM', IGM, IGMFormatado);

    // Captura e formata os dados do IGG
    ...
    const IGGFormatado = `<b>IMUNOGLOBULINA (IGG):</b> [${IGG}]<br>`;
    adicionarExame('IGG', IGG, IGGFormatado);

    // Captura e formata os dados do Fator Reumatoide
    ...
    const fatorreumatoideFormatado = `<b>FATOR REUMATOIDE:</b> [${fatorreumatoide}]<br>`;
    adicionarExame('fatorreumatoide', fatorreumatoide, fatorreumatoideFormatado);
    ...
    // Captura e formata os dados da Baciloscopia (BAAR)
    const BAARElement = document.getElementById('BAAR');
    const BAAR = BAARElement ? BAARElement.innerHTML : 'Não encontrado';

    // Formata os dados da Baciloscopia (BAAR)
    const BAARFormatado = `<b>BACILOSCOPIA (BAAR):</b> [${BAAR}]<br>`;
    adicionarExame('BAAR', BAAR, BAARFormatado);

    // Captura e formata os dados da Bacterioscopia (GRAM)
    const GRAMElement = document.getElementById('GRAM');
    const GRAM = GRAMElement ? GRAMElement.innerHTML : 'Não encontrado';

    // Formata os dados da Bacterioscopia (GRAM)
    const GRAMFormatado = `<b>BACTERIOSCOPIA (GRAM):</b> [${GRAM}]<br>`;
    adicionarExame('GRAM', GRAM, GRAMFormatado);

    // Captura e formata os dados da Cultura de Bactérias Aeróbias
    const CBAElement = document.getElementById('CBA');
    const CBA = CBAElement ? CBAElement.innerHTML : 'Não encontrado';

    // Formata os dados da Cultura de Bactérias Aeróbias
    const CBAFormatado = `<b>CULTURA DE BACTÉRIAS AERÓBIAS:</b> [${CBA}]<br>`;
    adicionarExame('CBA', CBA, CBAFormatado);

    // Captura e formata os dados da Pesquisa de Fungos
    const PFElement = document.getElementById('PF');
    const PF = PFElement ? PFElement.innerHTML : 'Não encontrado';

    // Formata os dados da Pesquisa de Fungos
    const PFFormatado = `<b>PESQUISA DE FUNGOS:</b> [${PF}]<br>`;
    adicionarExame('PF', PF, PFFormatado);

    // Captura e formata os dados da Cultura de Fungos
    const CFElement = document.getElementById('CF');
    const CF = CFElement ? CFElement.innerHTML : 'Não encontrado';

    // Formata os dados da Cultura de Fungos
    const CFFormatado = `<b>CULTURA DE FUNGOS:</b> [${CF}]<br>`;
    adicionarExame('CF', CF, CFFormatado);
